...
def get_projections_df(intervention_type):
...
def get_county_projections(intervention_type):
    # for each state in our data look at the results we generated via run.py
    # to create the projections
...
def get_usa_by_county_with_projection_df(intervention_type):
    ...
    projections_df = get_county_projections(intervention_type)
...
def get_usa_by_states_df(intervention_type):

    us_only = get_usa_by_county_df()
    abbrev_df = get_abbrev_df()
    interventions_df = get_interventions_df()
    projections_df = get_projections_df(intervention_type)
...
def get_usa_state_shapefile(shp, shx, dbf, intervention_type):
    ...
    join_and_output_shapefile(get_usa_by_states_df(intervention_type),
        shapefile.Reader(f'{public_data_path}/data/shapefiles-uscensus/tl_2019_us_state'),
        'STATEFP', 'State/County FIPS Code', shp_writer)

def get_usa_county_shapefile(shp, shx, dbf, intervention_type):
    shp_writer = shapefile.Writer(shp=shp, shx=shx, dbf=dbf)
    public_data_url = get_public_data_base_url()
    public_data_path = _file_uri_to_path(public_data_url)

    join_and_output_shapefile(get_usa_by_county_with_projection_df(intervention_type),
